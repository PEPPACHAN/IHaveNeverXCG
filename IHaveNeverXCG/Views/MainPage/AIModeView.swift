...
struct AIModeView: View {
    ...
    @State private var customCardsInfo: [CustomCards] = []
                        ...
                        if (modeName != "" && selectedMode != "" && numberOfCards != "" && tags != ""){
                            if purchase.hasUnlockedPro {
                                ...
                                withAnimation(.easeInOut(duration: 0.3)){
                                    mode.createCustom(cardText: tags, cardsCount: Int(numberOfCards) ?? 0, lang: language, userToken: uuid)
                                    if isFirstAiGame {
                                        showRating = true
                                    }
                                }
